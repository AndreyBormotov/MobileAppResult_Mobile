...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПарольЦентральнойБазы = Константы.ЛогинЦентральнойБазы.Получить();
КонецПроцедуры
#КонецОбласти
...
Функция ПолучитьНастройкиПодключения()
	...
	// http://localhost/homework15.2/hs/mobile/ping
	...
	АдресБезПротокола = Сред(АдресЦентральнойБазы, ПозицияРазделителя + 3); // localhost/homework15.2
	
	ПозицияРазделителя = СтрНайти(АдресБезПротокола, "/");
	
	Результат.АдресСервера = Лев(АдресБезПротокола, ПозицияРазделителя - 1); // localhost
	Результат.ПутьНаСервере = Сред(АдресБезПротокола, ПозицияРазделителя); // /homework15.2
