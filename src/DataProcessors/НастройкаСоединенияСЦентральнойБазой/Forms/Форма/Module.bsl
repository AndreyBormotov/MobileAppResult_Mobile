...
Процедура ОтправитьДанныеДоставок()
	...
	ТелоЗапроса = ПодготовитьТелоЗапроса(ИдентификаторКлиента);
...
Функция ПодготовитьТелоЗапроса(ИдентификаторКлиента)
	...
	Запрос.Текст = "ВЫБРАТЬ
					...
					|	ЕСТЬNULL (ДоставкаИзменения.Ссылка.Фотография, Неопределено) КАК Фотография,
...
//Добавляем документы для отправки
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Фотография) Тогда
			СтрокаФото = Base64Строка(Выборка.Фотография.Получить());
		КонецЕсли;

		ДанныеДокумента.Вставить("Фотография", СтрокаФото);
